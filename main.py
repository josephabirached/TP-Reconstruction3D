...
def CameraCalibration():
    ...
    objp[:, :2] *= 40
    #################################################
    # Tableau pour stocké les points des objets et les points des images de tous les images.
    objpoints = []  # 3d point dans le real world space
    imgpoints = []  # 2d points dans le plan d'image
    ############ to adapt ##########################
    ...
    for fname in images:
        ...
        # Retourne tout les corners de l'échiquier dans nos photos
        ############ to adapt ##########################
        ret, corners = cv.findChessboardCorners(gray, (4, 6), None)
        #################################################
        print(ret)
        # Si il existe on ajoute les points d'objet ou d'image
        if ret == True:
            objpoints.append(objp)
            corners2 = cv.cornerSubPix(gray, corners, (11, 11), (-1, -1), criteria)
            imgpoints.append(corners)
            # On dessine sur l'échiquier des lines
            ############ to adapt ##########################
    ...
    # Trouve les paramètres de la camera de plusieurs points de vue pour un pattern de calibration
...
def DepthMapfromStereoImages():
    ...
    speckleRange = 32)
    # Calcul la profondeur d'une image
...
def StereoCalibrate(Cameramtx):
    ...
    return pts1, pts2, F, maskF,E, maskE
